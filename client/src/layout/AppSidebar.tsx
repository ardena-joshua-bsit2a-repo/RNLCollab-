...
const Tooltip = ({
  ...
  <div className="relative group/tip w-full">
    ...
    <div className="
      ...
      px-3 py-2 text-xs font-medium shadow-2xl
      opacity-0 translate-x-[-6px]
      group-hover/tip:opacity-100 group-hover/tip:translate-x-0
      transition-all duration-150
    ">
      {label}
      <div className="absolute right-full top-1/2 -translate-y-1/2 border-[5px] border-transparent border-r-white dark:border-r-slate-900" />
    </div>
  </div>
);
...
const AppSidebar = () => {
  ...
  const sidebarItems: NavItem[] = [
...

    {
      ...
      children: [
        ...(isSuperAdmin
          ? [{ text: "Event Venue", path: "/venue", icon: <FiMapPin /> }]
          : []),
        { text: "All Events", path: "/events", icon: <FiCalendar /> },
        ...(isSuperAdmin
          ? [{ text: "Event Approvals", path: "/event-approval", icon: <FiShield /> }]
          : []),
      ],
    },
  ...
  // Shared inactive link classes
  const inactiveLink = "text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/80 dark:hover:text-white";
  const activeLink   = "bg-indigo-600 text-white shadow-lg shadow-indigo-600/25";
  const activeChild  = "bg-indigo-500/15 text-indigo-600 dark:text-indigo-400 font-medium";
  ...
  return (
    <>
      ...
      `}>
        ...
        <div className="h-[73px] shrink-0 border-b border-slate-200 dark:border-slate-800" />
                ...
                isOpen ? (
                  <>
                    <button
                      ...
                      className={`flex w-full items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${inactiveLink}`}
                    >
                    ...
                    <div className={`overflow-hidden transition-all duration-300 ${openMenus[item.text] ? "max-h-60" : "max-h-0"}`}>
                      <div className="ml-3 mt-0.5 space-y-0.5 border-l border-slate-200 dark:border-slate-800 pl-3 pb-1">
                        {item.children.map((child, ci) => (
                          <NavLink
                            ...
                            className={({ isActive }) => `
                              ...
                              transition-all duration-200
                              ${isActive ? activeChild : inactiveLink}
                            `}
                          >
                  ...
                  <div className="space-y-0.5">
                    <Tooltip label={item.text}>
                      <div className="flex w-full items-center justify-center py-2 rounded-xl text-base text-slate-400 dark:text-slate-600 cursor-default select-none">
                        {item.icon}
                      </div>
                    </Tooltip>
        ...
        <div className="border-t border-slate-200 dark:border-slate-800 shrink-0 p-3">
          {isOpen ? (
            <div className="rounded-xl bg-slate-100 dark:bg-slate-900 px-3 py-2.5">
              <p className="text-[10px] text-slate-400 dark:text-slate-500 whitespace-nowrap">
                Event Management System
              </p>
              <p className="text-xs font-semibold text-slate-900 dark:text-white mt-0.5 whitespace-nowrap">
